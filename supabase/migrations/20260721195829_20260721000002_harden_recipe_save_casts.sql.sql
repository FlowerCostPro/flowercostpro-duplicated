/*
# Make recipe save robust against bad casts

## What changed
- save_owner_arrangement_recipe: guard quantity/portionDivisor casts with COALESCE + NULLIF
  so NaN/empty strings don't abort the whole insert.
- update_owner_arrangement_recipe: same guards.
- get_owner_arrangement_recipes: unchanged (already returns portion_divisor).

## Security
- Functions remain SECURITY DEFINER, search_path = public.
- Re-execute grants revoked from anon/public to preserve lockdown.
*/
