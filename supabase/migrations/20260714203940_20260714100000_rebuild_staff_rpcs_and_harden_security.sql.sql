/*
# Rebuild staff working-budget RPC + security hardening

## 1. get_working_budget_for_staff — rebuilt to return TABLE type
PostgREST handles jsonb-returning functions inconsistently across versions.
Switching to a RETURNS TABLE type with a single numeric column makes the
result unambiguously an array of rows, which the supabase-js client handles
the same way every time: data[0].working_budget.

The function:
  - Verifies the caller is a staff account (owner_id IS NOT NULL)
  - Reads the owner's labor_percent from markup_settings (bypasses RLS via
    SECURITY DEFINER)
  - Computes working_budget = customer_budget × (1 − labor_percent / 100)
  - Returns ONLY the working_budget — never the labor percentage

## 2. save_staff_order — remove profit from return value
The original design spec said staff should never see profit figures.
A previous fix added profit to the return for convenience, but that violates
the security principle. Remove it; the frontend doesn't need it for the
staff flow (staff see retail price and their order, not profit).

## 3. get_staff_saved_orders — remove profit from return value
Same principle: staff should not see the owner's profit figures.
*/

-- ─── 1. Rebuild get_working_budget_for_staff ──────────────────────────────

DROP FUNCTION IF EXISTS get_working_budget_for_staff(numeric);
...
-- ─── 2. save_staff_order — remove profit from return ──────────────────────
...
BEGIN
  ...
  -- Return only safe fields: no wholesale, no profit
...
-- ─── 3. get_staff_saved_orders — remove profit from return ────────────────
