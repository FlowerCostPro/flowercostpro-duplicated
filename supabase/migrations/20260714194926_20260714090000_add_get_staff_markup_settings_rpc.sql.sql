/*
# Add get_staff_markup_settings RPC
...
BEGIN
  ...
  INTO v_owner_id  -- reuse var as result holder is not possible; use a separate
  FROM markup_settings ms
  WHERE ms.user_id = get_owner_id_for_user(auth.uid());

  RETURN COALESCE(v_owner_id::text::jsonb, jsonb_build_object(
