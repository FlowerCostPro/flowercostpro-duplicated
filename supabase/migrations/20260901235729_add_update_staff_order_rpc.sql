-- Add update_staff_order function
-- Staff can update orders belonging to their owner (where staff_id matches their auth.uid).
-- This mirrors update_owner_order but uses get_owner_id_for_user for access control
-- and recalculates wholesale/retail/labor server-side (staff never sends wholesale costs).
...
BEGIN
  ...
  PERFORM 1 FROM orders WHERE id = p_order_id AND user_id = v_owner_id;
  IF NOT FOUND THEN
  ...
  FOR v_elem IN SELECT * FROM jsonb_array_elements(p_products) LOOP
    ...
    IF v_raw_ws IS NULL THEN
      ...
      v_ws     := 0;  -- staff never sends wholesale
      v_divisor := COALESCE((v_elem->>'portion_divisor')::int, 1);
