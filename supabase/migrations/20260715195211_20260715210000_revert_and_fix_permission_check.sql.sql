/*
# Revert generate_pos_text to working version + fix permission check

## What this does
1. Reverts the function body to the exact version from migration
   20260715170000 that produced correct retail prices, labor line,
   and summing total from order history.

2. Makes exactly ONE change to that reverted version: the permission
   check now allows both the shop owner (auth.uid()) and staff
   accounts (get_owner_id_for_user) whose owner_id matches the shop
   that owns the order.
...
This sets v_owner_id to the owner's UUID for both staff (via
get_owner_id_for_user) and the owner themselves (via auth.uid()
fallback). Everything else — pricing, text building, labor, total —
is unchanged and uses v_owner_id exactly as the original did.
*/
...
BEGIN
  -- Verify caller belongs to the shop that owns the order:
  --   Staff → get_owner_id_for_user returns their owner's UUID
  --   Owner → returns NULL, fall back to their own UUID
  v_owner_id := get_owner_id_for_user(auth.uid());
  IF v_owner_id IS NULL THEN
    v_owner_id := auth.uid();
  END IF;
