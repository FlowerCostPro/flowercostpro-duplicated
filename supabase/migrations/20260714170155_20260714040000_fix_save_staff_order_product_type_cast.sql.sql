/*
# Fix save_staff_order: product_type enum cast error

## Problem
The `save_staff_order()` SECURITY DEFINER function declared its local variable
`v_ptype` as `text` and selected `pt.type::text` into it. When that text value
was subsequently inserted into `order_products.type` (a `product_type` enum
column), PostgreSQL raised:

  ERROR: column "type" is of type product_type but expression is of type text

This only affected staff accounts (owners insert via PostgREST, which handles
the enum cast automatically). Every order created by a staff member failed to
save.

## Fix
1. Declare `v_ptype` as `product_type` (the enum type) instead of `text`.
2. Select `pt.type` directly (no `::text` cast) into `v_ptype`.
3. The INSERT into `order_products` now receives a proper enum value.

## Security
No security changes — this is a pure type-cast fix inside an existing
SECURITY DEFINER function. RLS policies and function permissions are unchanged.
*/
