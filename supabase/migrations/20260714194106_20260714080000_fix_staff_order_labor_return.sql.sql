/*
# Fix staff order not deducting labor

## Problem
The `save_staff_order()` RPC correctly computes `labor_amount` and
`profit` server-side (using the owner's `markup_settings.labor_percent`)
and stores them in the `orders` table. However, the RPC's return value
only included `order_id`, `total_retail`, and `created_at` — NOT
`labor_amount` or `profit`.

The frontend `saveOrder` function (useSupabaseData.ts) built the local
`OrderRecord` using `order.laborAmount` from the `OrderBuilder`, which
was `null` for staff because:
  - Staff cannot read `markup_settings` (RLS blocks them — policies
    require `user_id = auth.uid()`, but the rows belong to the owner).
  - Without `laborPercent`, `OrderBuilder` computes `laborAmount = null`.

So the saved order in local state had `laborAmount: null`, making it
appear as though labor was never deducted — even though the DB had the
correct value.

## Fix
1. Drop the old `save_staff_order` overload (without `p_photo`) so
   PostgREST resolves to the correct version.
2. Update the RPC to also return `labor_amount` and `profit` in its
   response, so the frontend can use the server-computed values.
*/

DROP FUNCTION IF EXISTS save_staff_order(text, text, text, text, numeric, jsonb);
