...
import { Archive, Eye, Calendar, Copy, Search, Import as SortAsc, Image as ImageIcon } from 'lucide-react';
...
const StaffSavedOrders: React.FC<StaffSavedOrdersProps> = ({ orders, selectedOrderId }) => {
  ...
  const handleCopy = async (order: OrderRecord) => {
    ...
    try {
      const { imageCopied } = await copyTextAndPhoto(text, order.photo);
