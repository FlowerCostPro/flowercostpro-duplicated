...
const LowStockAlert: React.FC<LowStockAlertProps> = ({ templates, onUpdateTemplate }) => {
  ...
  const handleRestock = (template: ProductTemplate) => {
    const amount = parseInt(restockAmounts[template.id] || '0');
  ...
  const handleSetStock = (template: ProductTemplate) => {
    const amount = parseInt(restockAmounts[template.id] || '0');
    if (amount < 0 || restockAmounts[template.id] === undefined) return;
  ...
  const ItemCard = ({ template, isOutOfStock }: { template: ProductTemplate; isOutOfStock: boolean }) => (
    <div className={`border rounded-lg p-4 relative ${isOutOfStock ? 'border-red-300 bg-red-50' : 'border-amber-300 bg-amber-50'}`}>
      <button
        onClick={() => handleDismiss(template.id)}
      ...
      <div className="flex gap-2">
        ...
        />
        <button
          onClick={() => isOutOfStock ? handleSetStock(template) : handleRestock(template)}
          disabled={!restockAmounts[template.id]}
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-md p-6 border border-gray-200">
        ...
        <p className="text-gray-600 text-sm">
          Restock items directly from this page or dismiss alerts temporarily. Inventory updates instantly across the app.
        </p>
      ...
      {outOfStock.length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {outOfStock.map(t => <ItemCard key={t.id} template={t} isOutOfStock={true} />)}
          </div>
        </div>
      )}
      ...
      {lowStock.length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {lowStock.map(t => <ItemCard key={t.id} template={t} isOutOfStock={false} />)}
          </div>
        </div>
      )}
    </div>
